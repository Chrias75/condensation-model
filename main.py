...
# hydraulic diameter from geometry
d_h = (4 * b * h) / (2 * b + 2 * h)
...
for i, item in enumerate(re):
    ...
    def func(r):
        ...
        if flow_direction == 'horizontal':
            ...
            try:
                f_s[i] = a * f_surf_tens_ei(r, surf_tens[i], theta_a, theta_m, beta[i])
            except TypeError:
                f_s[i] = a * f_surf_tens_ei(r, np.array([surf_tens])[i], theta_a, theta_m, beta[i])
            except IndexError:
                f_s[i] = a * f_surf_tens_ei(r, np.array([surf_tens])[i], theta_a, theta_m, beta[i])
